...
class TopicProgress extends StatelessWidget {
  ...
  Widget _progressCount(User user, Topic topic) {
    if (user != null && topic != null) {
      return Padding(
        padding: const EdgeInsets.only(left: 8),
        child: Text(
          '${user.topics[topic.id]?.length ?? 0} / ${topic?.quizzes?.length ?? 0}',
  ...
  double _calculateProgress(Topic topic, User user) {
    try {
      int totalQuizzes = topic.quizzes.length;
      int completedQuizzes = user.topics[topic.id].length;
